...
export default function App() {
  ...
  // Helper to scroll to a specific match and visually highlight it
  const scrollToMatch = (index: number) => {
    ...
    // Apply active/inactive styles to match elements
    for (let i = 0; i < matches.length; i++) {
      const el = matches[i] as HTMLElement;
      if (i === targetIndex) {
        el.style.backgroundColor = '#fbbf24'; // bg-amber-400
        el.style.outline = '2px solid #d97706'; // outline-amber-600
        el.style.borderRadius = '2px';
      } else {
        el.style.backgroundColor = '#fef08a'; // bg-yellow-200
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <form onSubmit={handleSearchSubmit} className="grow max-w-md relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-32 sm:pr-36 py-1.5 md:py-2 bg-slate-50 border border-slate-200 focus:border-blue-500 focus:bg-white text-xs md:text-sm rounded-xl transition-all outline-hidden focus:ring-1 focus:ring-blue-500"
            />
            <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
              {searchQuery && searchMatchesCount > 0 && (
                <div className="flex items-center gap-0.5 bg-slate-100 border border-slate-200/60 px-1.5 py-0.5 rounded-lg text-[10px] md:text-xs font-semibold text-slate-600">
                  <span className="tabular-nums">{currentMatchIndex + 1}/{searchMatchesCount}</span>
              ...
              {searchInput && (
                <button
                  type="button"
                  onClick={clearSearch}
                  className="p-1 hover:bg-slate-200 rounded-md text-slate-400 hover:text-slate-600 text-xs transition-colors cursor-pointer"
                  title="Clear search"
                >
                  <X className="w-3 h-3" />
                </button>
              ...
              <button
                type="submit"
                className="bg-blue-700 hover:bg-blue-800 text-white font-semibold text-2xs md:text-xs px-2.5 py-1 rounded-lg transition-colors cursor-pointer shrink-0"
              >
                {t.searchButton}
              </button>
            </div>
